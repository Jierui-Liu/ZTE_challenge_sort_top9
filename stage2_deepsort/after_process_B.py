...
def after_process(dir_txt,txt,s,max_gap=20):
    ...
    # cnt=0
    # bbox_cnt=0
    # if txt in difficult_txt0:
    #     for o_key in object_dict.keys():
    #         num=len(object_dict[o_key])
    #         if num<13 and object_dict[o_key][0]>25 and object_dict[o_key][-1]<M[-1,0]-25:
    #             a=object_dict[o_key]
    #             cnt=cnt+1
    #             print(o_key,object_dict[o_key])
    ...
    to_delete=[]
    for o_key in object_dict.keys():
        # if txt=='Track5.txt' and o_key==43:
        #     continue
        ...
        for i in range(len(object_dict[o_key][:-1])):
            ...
            if area_now>0.3*area:
                ...
                continue
            if next==start+1 or next-start>max_gap or iou_gap<0.4/max_gap+0.1:
                continue
...
if __name__=="__main__":
    ...
    # label='c2_rec_mmdet_81.9_4_score0.7_iout0.55_B'
    # label='c2_rec_mmdet_82.1_4_score0.7_iout0.55_B'
    # label='c2_B_rec_74.8_3_score0.7_iout0.55_B_75.592'
    label='c2_B_rec_74.8_4_score0.7_iout0.55_B'
    dir_read='./demo/A-track/'+label
    # dir_save='./demo/A-track/after_rec_dif_i30'+label
    dir_save='./demo/A-track/after_rec_dif_i30'+label
    ...
    for txt in txts:
        print(txt)
        frame_dict,M_out=after_process(dir_read,txt,size[txt],max_gap=30)
