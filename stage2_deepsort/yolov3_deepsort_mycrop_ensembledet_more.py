...
class VideoTracker(object):
    ...
    def run(self,id_video):
        ...
        dirtxt_read='/home/liujierui/proj/FairMOT-master/outputs/A-track_all_hrnet_v2_w18_score0.3_nms0.3_conf0.4'
        ...
        # dirtxt_read='/home/liujierui/proj/centerNet-deep-sort-master/demo/A-track/cn_score0.4_ensemble_fair0.4'
        ...
        dirtxt_read1='/home/liujierui/proj/FairMOT-master/outputs/A-track_all_dla34_score0.3_nms0.3_conf0.5'
        txt_read1=join(dirtxt_read1,'Track'+str(id_video)+'.txt')
        M1=file2array(txt_read1)

        frame_dict1={}
        for f in range(1,int(M1[-1,0])+1):
            frame_dict1[f]={}#   以帧id为索引,物体id为次级索引
        for i in range(len(M1)):
            l=M1[i]
            frame_dict1[int(l[0])][int(l[1])]=l.copy()
        ...
        for id_img in filelist:
            ...
            # ensemble1
            if len(frame_dict1[id_int].keys())==0:
                pass
            else:
                new=np.zeros((1,4))
                already=np.zeros((1,4))
                for o_key in frame_dict1[id_int].keys():
                    new[0,:]=frame_dict1[id_int][o_key][2:6]
                    skip=False
                    for o_key_already in frame_dict[id_int].keys():
                        already[0,:]=frame_dict[id_int][o_key_already][2:6]
                        iou=get_iou(new[0],already[0])
                        if iou>0.5:
                        # if iou>0.7:
                            skip=True
                            break
                    if skip:
                        continue
                    cls_conf_other.append(frame_dict1[id_int][o_key][6])
                    bbox_xywh_other=np.vstack((bbox_xywh_other,new.copy()))
            if bbox_xywh_other is not None:
...
if __name__=="__main__":
    ...
    # label='init_score0.8_nms0.3_1.0_crop_ensemble_hrnet_fair0.35_'
